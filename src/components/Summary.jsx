...
function Summary({userAnswer}) {
    ...
    const skippedPercent = Math.round((skippedAnswers.length/userAnswer.length)*100)
    const correctPercent = Math.round((correctAnswers.length/userAnswer.length)*100)
    const wrongPercent = 100 - skippedPercent-correctPercent


  return (
    <div id="summary">
      <img src={Quiz_over} alt="" />
      ...
      <ol>
        {userAnswer.map((answer,index)=>{
            ...
            <p className={cssClass}>{answer===null?'No answer were picked':`${answer}`}</p>
          </li>)
        })}
      </ol>
    </div>
